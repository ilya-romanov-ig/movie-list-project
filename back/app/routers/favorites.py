...
async def add_favorite_film_route(
    ...
    await add_favorite_film(db, user_id=current_user.user_id, film_id=film_id)
...
async def remove_favorite_film_route(
    ...
    ok = await remove_favorite_film(db, user_id=current_user.user_id, film_id=film_id)
    if not ok:
...
async def list_favorite_films(
    ...
    films = await get_favorite_films(db, current_user.user_id)
    return {
        "count": len(films),
        "items": [
...
async def add_favorite_actor_route(
    ...
    await add_favorite_actor(db, user_id=current_user.user_id, actor_id=actor_id)
...
async def remove_favorite_actor_route(
    ...
    ok = await remove_favorite_actor(db, user_id=current_user.user_id, actor_id=actor_id)
    if not ok:
        raise HTTPException(404, "Not in favorites")
...
async def list_favorite_actors(
    ...
    actors = await get_favorite_actors(db, current_user.user_id)
    return {
        "count": len(actors),
        "items": [
